...
use commonware_runtime::{Metrics as RuntimeMetrics, Runner, tokio};
...
use p2p_node::metrics::Metrics as NodeMetrics;
...
fn main() {
    ...
    let rt_config = tokio::Config::new().with_storage_directory(&config.directory);
    ...
    executor.start(|context| async move {
        ...
        tokio::telemetry::init(context.clone(), logging, Some(metrics_addr), None);

        let metrics_ctx = context
            .with_label("rlnc")
            .with_attribute("strategy", &config.strategy)
            .with_attribute("role", &config.role);
        let node_metrics = NodeMetrics::init(metrics_ctx);
        ...
        let signer = ed25519::PrivateKey::read_cfg(&mut key_bytes.as_slice(), &())
        ...
        for entry in &peer_entries {
            ...
            let pk = ed25519::PublicKey::read_cfg(&mut pk_bytes.as_slice(), &())
                .expect("invalid peer public key");

            if pk != my_pk {
                let ingress: Ingress = entry.address.parse::<SocketAddr>().map_or_else(
                    |_| {
                        let (host, port_str) = entry
                            .address
                            .rsplit_once(':')
                            .expect("peer address must be host:port");
                        let port: u16 = port_str
                            .parse()
                            .expect("peer address port must be a number");
                        Ingress::Dns {
                            host: Hostname::new(host).expect("invalid peer hostname"),
                            port,
                        }
                    },
                    Into::into,
                );
                bootstrappers.push((pk.clone(), ingress));
                peer_pks.push(pk);
            }
        }

        // Create P2P network.
        let listen: SocketAddr = format!("0.0.0.0:{}", config.port).parse().unwrap();
        let dialable: Ingress = if let Some(addr) = &config.dialable_address {
            addr.parse::<SocketAddr>().map_or_else(
                |_| {
                    let (host, port_str) = addr
                        .rsplit_once(':')
                        .expect("dialable_address must be host:port");
                    let port: u16 = port_str
                        .parse()
                        .expect("dialable_address port must be a number");
                    Ingress::Dns {
                        host: Hostname::new(host).expect("invalid dialable hostname"),
                        port,
                    }
                },
                Into::into,
            ...
            let sa: SocketAddr = format!("127.0.0.1:{}", config.port).parse().unwrap();
            sa.into()
        };
        ...
        let (mut network, mut oracle) = discovery::Network::new(context.clone(), p2p_cfg);
        ...
        let mut all_pks: Vec<ed25519::PublicKey> = peer_entries
            ...
            .map(|e| {
                let pk_bytes =
                    hex::decode(e.public_key.strip_prefix("0x").unwrap_or(&e.public_key)).unwrap();
                ed25519::PublicKey::read_cfg(&mut pk_bytes.as_slice(), &()).unwrap()
            })
            .collect();
        all_pks.sort();
        let peer_set =
            commonware_utils::ordered::Set::try_from(all_pks).expect("duplicate peer public keys");
        ...
        match config.strategy.as_str() {
            ...
            "pedersen" => {
                ...
                let bls_pk = bls12381::PublicKey::read_cfg(&mut pk_bytes.as_slice(), &())
                ...
                let signer_opt = config.proposer_signing_key.as_ref().map(|sk_hex| {
                    ...
                    let mut sk_rng = rand_chacha::ChaCha20Rng::from_seed(seed);
                    PedersenSigner::<bls12381::PrivateKey>::generate(&mut sk_rng)
                });

                let proto =
                    PedersenProtocol::<G1, bls12381::PrivateKey, N>::new(m, bls_pk, signer_opt);
                let mut actor = NodeActor::new(
                    proto,
                    ann_tx,
                    ann_rx,
            ...
            "bfkw" => {
                ...
                let proto = BfkwProtocol::<Bls12381, N>::new(m, g1_pk, signer_opt);
                let mut actor = NodeActor::new(
                    proto,
                    ann_tx,
                    ann_rx,
